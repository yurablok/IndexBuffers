...
#include "protobuf/addressbook_v3.pb.h"


float run_protobuf_v3(const uint32_t reps) {
    ...
    for (uint32_t i = 0; i < reps; ++i) {
        proto_v3::AddressBook addressBookSrc;
        {
            ...
            alicePhones->set_type(proto_v3::Person_PhoneType_MOBILE);
            ...
            bobPhones->set_type(proto_v3::Person_PhoneType_HOME);
            bobPhones = bob->add_phones();
            bobPhones->set_number("555-7654");
            bobPhones->set_type(proto_v3::Person_PhoneType_WORK);
        ...
        proto_v3::AddressBook addressBookDst;
        {
            ...
            for (uint8_t personIt = 0, personSize = addressBookDst.people_size();
                ...
                case proto_v3::Person::EmploymentCase::kUnemployed:
                    sum += 'unem';
                    break;
                case proto_v3::Person::EmploymentCase::kEmployer:
                    sum += 'empl';
                    sum += person.employer().size();
                    break;
                case proto_v3::Person::EmploymentCase::kSchool:
                    sum += 'scho';
                    sum += person.school().size();
                    break;
                case proto_v3::Person::EmploymentCase::kSelfEmployed:
                    sum += 'self';
                    break;
                default:
                    break;
